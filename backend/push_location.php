...
if ($ins->execute()) {
    ...
    if ($justReached) {
        $upd = $conn->prepare("UPDATE tasks SET 
            ...
            WHERE id = ?");
        $upd->bind_param("dddisssdddi", $lat, $lng, $distanceMeters, $insideSec, $destReachedAt, $targetDeadlineAt, $lat, $lng, $distanceMeters, $taskId);
    } else {
        $upd = $conn->prepare("UPDATE tasks SET 
            last_latitude = ?, 
            last_longitude = ?, 
            ...
            time_inside_destination_seconds = ?
            WHERE id = ?");
        $upd->bind_param("ddidii", $lat, $lng, $distanceMeters, $isInside, $insideSec, $taskId);
    }
    $upd->execute();
    $upd->close();
